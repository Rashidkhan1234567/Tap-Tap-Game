...
const Level_6Screen = ({ navigation }) => {
  ...
  useEffect(() => {
    const loadSounds = async () => {
      ...
      soundRef.current.gameFinish = await Audio.Sound.createAsync(
        require("../../assets/sounds/game-finish.mp3")
      );
    };
  ...
  useEffect(() => {
    ...
    if (number === 0) {
      ...
      soundRef.current.gameFinish.sound?.playAsync();
      soundRef.current.countdown.sound?.stopAsync();
      setTimeout(() => {
        setAlertTitle("Congratulations!");
        setAlertMessage("You Win!");
        setAlertMessage(
          "The game has been completed! If you want to play again, click OK to return to the Home screen and restart the game."
        );
        setShowAlert(true);
